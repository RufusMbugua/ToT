 ...
 */
use application\models\Entities\E_Groups;
...
class M_Groups extends MY_Model {
	...
	function getUser() {
		...
		if ($this -> input -> post()) {//check if a post was made
			...
			$user = $this -> em -> getRepository('models\Entities\E_Groups') -> findOneBy(array('email' => $this -> input -> post('email'), 'password' => $this -> input -> post('password')));
	...
	function addRecord() {
		...
		if ($this -> input -> post()) {//check if a post was made
			...
			for ($i = 1; $i <= $this -> noOfInsertsBatch; ++$i) {
				...
				$this -> theForm = new \models\Entities\E_Groups();
	...
	function viewRecords() {
		try {
			$query = $this -> em -> createQuery('SELECT u FROM models\Entities\E_Groups u');
