 ...
 */
use application\models\Entities\E_Project;

class M_Project extends MY_Model {
	var $isUser, $email, $userRights, $affiliation;
	var $id, $attr, $frags, $elements, $theIds, $noOfInserts, $batchSize, $project;
	...
	function getUser() {
		...
		if ($this -> input -> post()) {//check if a post was made
			...
			$user = $this -> em -> getRepository('models\Entities\E_Project') -> findOneBy(array('email' => $this -> input -> post('email'), 'password' => $this -> input -> post('password')));
	...
	function addRecord() {
		...
		if ($this -> input -> post()) {//check if a post was made
			...
			for ($i = 1; $i <= $this -> noOfInsertsBatch; ++$i) {
				...
				$this -> theForm = new \models\Entities\E_Project();
	...
	function viewRecords() {
		try {
			$query = $this -> em -> createQuery('SELECT u FROM models\Entities\E_Project u');
